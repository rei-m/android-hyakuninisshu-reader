...
data class KarutaExams(private val values: List<KarutaExam>) {
     ...
     */
    val totalWrongKarutaNos: KarutaNos =
        KarutaNos(values.asSequence().fold(mutableSetOf<KarutaNo>()) { karutaNoSet, karutaExam ->
            karutaNoSet.addAll(karutaExam.result.wrongKarutaNos.values)
            karutaNoSet
        }.toList())
